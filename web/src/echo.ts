...
function resend_message(
    ...
    message_store.mutable_for(message).update_resend(true);
...
export function edit_locally(message: Message, request: LocalEditRequest): Message {
    ...
    if (request.new_topic !== undefined || request.new_stream_id !== undefined) {
        ...
        if (new_stream_id !== undefined) {
            message_store.mutable_for(message).update_stream_id(new_stream_id);
        }
    ...
    if (message_content_edited) {
        ...
        if (request.content !== undefined) {
            ...
            message_store.mutable_for(message).update_mentioned(request.mentioned ?? false);
            message_store.mutable_for(message).update_mentioned_me_directly(
                request.mentioned_me_directly ?? false,
            );
            message_store.mutable_for(message).update_alerted(request.alerted ?? false);
        } else {
            ...
            message_store.mutable_for(message).update_is_me_message(is_me_message);
            if (request.starred !== undefined) {
                message_store.mutable_for(message).update_starred(request.starred);
            }
            if (request.historical !== undefined) {
                message_store.mutable_for(message).update_historical(request.historical);
            }
            if (request.collapsed !== undefined) {
                message_store.mutable_for(message).update_collapsed(request.collapsed);
            }
        }
    }
...
export let reify_message_id = (local_id: string, server_id: number): void => {
    ...
    message_store.mutable_for(message).update_id(server_id);
    message_store.mutable_for(message).update_locally_echoed(false);
...
export function process_from_server(messages: ServerMessage[]): ServerMessage[] {
    ...
    for (const message of messages) {
        ...
        // the backend.
        message_store.mutable_for(client_message).update_timestamp(message.timestamp);

        message_store.mutable_for(client_message).update_topic_links(
            message.topic_links ?? [],
        );
        message_store.mutable_for(client_message).update_is_me_message(
            message.is_me_message,
        );
        message_store.mutable_for(client_message).update_submessages(message.submessages);
...
export function display_slow_send_loading_spinner(message: Message): void {
    ...
    if (message.locally_echoed && !message.failed_request) {
        message_store.mutable_for(message).update_show_slow_send_spinner(true);
