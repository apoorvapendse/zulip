...
export function is_tenor_enabled(): boolean {
    return (
        realm.tenor_api_key !== "" &&
        realm.realm_gif_rating !== realm.gif_rating_options.disabled.id
    );
}
...
export function get_rating(): GifRating {
    ...
    for (const rating of ["pg", "g", "pg-13", "r"] as const) {
        if (options[rating]?.id === realm.realm_gif_rating) {
            return rating;
        }
    }
    ...
    blueslip.error("Invalid gif_rating value: " + realm.realm_gif_rating);
    return "g";
}
...
export function update_gif_rating(): void {
    ...
    if (realm.realm_gif_rating === realm.gif_rating_options.disabled.id) {
        $(".zulip-icon-gif").hide();
    } else {
        $(".zulip-icon-gif").show();
    }
