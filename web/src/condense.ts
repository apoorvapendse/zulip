...
export function uncollapse(message: Message): void {
    ...
    message_store.mutable_for(message).update_collapsed(false);
...
export function collapse(message: Message): void {
    message_store.mutable_for(message).update_collapsed(true);
...
export function toggle_collapse(message: Message): void {
    ...
    if (message.collapsed) {
        if (is_condensable) {
            message_store.mutable_for(message).update_condensed(true);
            $content.addClass("condensed");
            show_message_expander($row);
        }
        uncollapse(message);
        ...
        if (is_condensed) {
            message_store.mutable_for(message).update_condensed(false);
            $content.removeClass("condensed");
            show_message_condenser($row);
        } else {
            collapse(message);
